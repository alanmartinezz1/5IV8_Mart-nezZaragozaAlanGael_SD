...
class ServerThread extends Thread {
    ...
    public void run() {
        //Mientras el server siga activo.
        while (!Server.serverSocket.isClosed()) {
            try {
                Server.Display("Esperando jugador...");
                Socket clientSocket = Server.serverSocket.accept();
                Server.Display("Jugador llegando...");
                SClient nclient = new SClient(clientSocket, Server.IdClient);
                Message msg = new Message(Message.Message_Type.Text);
                msg.playerNumber = Server.IdClient;	//Asignar numero a cada usuario entrante.
...
public class Server {
    ...
    // Damos el numero del puerto para empezar.
    ...
    // Envio de mensajes desde el servidor al cliente.
